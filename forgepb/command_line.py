...
@click.option('-rv', '--release-version', 'release_version', type=click.STRING, default=None, help='Release version of localnet')
@click.option('-m', '--moniker', 'moniker', type=click.STRING, default=None, help='A moniker to be used for spinning up a localnet node. Not required for testnet or mainnet')
@click.option('-cid', '--chain-id', 'chain_id', type=click.STRING, default=None, help='A chain id to be used for spinning up a localnet node. Not required for testnet or mainnet')
    ...
    if edit_config or network or save_loc or release_version or provenance_branch:
        ...
        # Use send args to create node
            ...
            if release_version and release_version not in utils.get_versions(provenance_path):
                ...
                builder.build(global_.CHAIN_ID_STRINGS[network], network, config, provenance_branch, release_version, list(boot_args), moniker, chain_id)
        else:
